...
本节从一个最小示例开始，演示如何创建可由BepInEx加载的Unity IL2CPP Mod，并在Unity生命周期中执行自定义代码。

## Mono和IL2CPP

Unity游戏常见的脚本后端包括Mono和IL2CPP。
...
IL2CPP采用另一条构建路线：Unity先把C#脚本的IL转换为C++代码，再由平台原生编译器（如MSVC、Clang）编译为机器码。在Windows平台上，游戏逻辑通常位于`GameAssembly.dll`中，运行时不再依赖Mono执行这些脚本IL。

逆向时面对的是原生机器码，而不是可直接反编译的游戏脚本IL。类名、方法名和类型结构等信息仍有一部分保存在全局元数据中，需要把元数据和`GameAssembly.dll`结合起来恢复可读的类型和函数映射。
...
- 如果游戏目录中存在：
    ...
    且没有`GameAssembly.dll`，那么它通常使用**Mono**。
...
## `global-metadata.dat`的作用

虽然IL2CPP把脚本编译成了本地代码，但Unity仍需保留类型和反射所需的元数据。`global-metadata.dat`包含类名、方法名、字段、参数类型和继承关系等信息。

这些信息不会以普通调试符号的形式直接附在`GameAssembly.dll`的本地函数上。Il2CppDumper会解析`global-metadata.dat`和`GameAssembly.dll`中的相关表结构，生成Dummy DLL、IDA脚本和类型定义，帮助恢复方法和地址之间的映射。编译器优化仍可能改变或合并部分本地代码，因此工具输出需要结合实际反汇编结果判断。
...
确保已经按照[开发入门](./getting_started.md)完成BepInEx安装，并至少进入过一次游戏主菜单。
...
右键项目下的**依赖项**，选择**添加项目引用**，再通过“浏览”添加所需的DLL。
...
对这三个引用，将“复制本地”设为`False`，避免把游戏和BepInEx自带的程序集一起复制到输出目录。

![image-20260111002504985](./hello_world_1.assets/image-20260111002504985.png)

将项目的输出路径改为`BepInEx/plugins`文件夹。
...
将`Class1.cs`重命名为`Plugin.cs`，并替换为以下代码：
...
确认构建成功后启动游戏。按下<kbd>F1</kbd>时，控制台中应输出`HelloWorld from my first IL2CPP Mod!`。

![image-20260111130151837](./hello_world_1.assets/image-20260111130151837.png)

到这里为止，我们已经完成了一个最基础的IL2CPP Mod：

它可以被BepInEx加载、在Unity生命周期中运行，并通过按键触发自定义逻辑。重点不是打印一行`HelloWorld`，而是建立一个可继续接入游戏状态和补丁逻辑的运行入口。
...
本节的重点是认识IL2CPP，并创建一个最基础的IL2CPP Mod：
...
初次实践时不必一次理解模板中的全部细节。例如，C语言初学者第一次接触下面的代码：
...
> 我在这个“模板代码”中的“恰当位置”写`printf`，就能让程序输出需要的内容。

可以先确认项目能够构建、加载和响应按键，再逐步检查每一行代码及其依赖。遇到不理解的类型或调用时，应结合框架文档、Interop程序集和运行日志验证。
